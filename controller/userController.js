...
const home = async (req,res)=>{
    ...
    if(req.session.user_id){
        ...
        res.render('userhome',{userData,bannerData,maleShoeCollection,categoryData});
    }else{
        const bannerData = await banner.find({is_delete:false})
        const categoryData = await category.find({is_delete:false})
...
const verifyOtp = async(req,res)=>{
    ...
    try {
        const phoneNumber = req.session.userData.phone;
...
const resendOtp = async(req,res)=>{
    try {
        ...
        if(currentdifference >= timeLimit){
            ...
            res.render('otp',{message:`otp regenerate only after ${inSeconds} seconds`})
        }
...
const FPsendOTP = async(req,res)=>{
    try {
        ...
        const phone = req.body.phone ;
        const country_code = req.body.country_code;
        const userData = await users.findOne({phoneNumber : req.body.phone })
        if(userData){
        ...
        res.render('FPotp')
            ...
            res.render('forgetpassword',{message: 'enter the verified phone number'})
        }
...
const newPassword = async(req,res)=>{
    try {
        ...
        const newPassword = await users.findOneAndUpdate({phoneNumber:phone},{$set:{password:spassword}});
        if(newPassword){
            res.redirect('/login')
        }else{
            res.render('newPassword')
...
//         if(req.session.user_id){
//             if(search){
//                 // const Category = await category.find({Title:{$regex: search,$options:'i'}})
...
//                 res.render('productListing',{productData,categoryData,userData,allSizes,id:req.query.id})
//             }else{
//                 if(sort){
...
//                         sortObj = {price:1}
//                     }
//                     await product.find({is_delete: false}).sort(sortObj).exec(function(error,data){
//                         if(error){
//                             console.log(error.message);
//                         }else{
//                             res.send(data)
...
//                 }else{
//                     const categoryData = await category.find({is_delete: false})
//                     const productData = await product.find({category:req.query.id,is_delete:false})
//                     const userData = await users.find({_id:req.session.user_id})
//                     const allSizes = await product.schema.path('sizes').enumValues
...
//                 res.render('productListing',{productData,categoryData,allSizes,id:req.query.id})
//             }else{
//                 if(sort){
//                     let sortValue = req.query.sort
//                     let sortObj = {}
...
//                         sortObj = {price:1}
//                     }
//                     await product.find({category:id}).sort(sortObj).exec(function(error,data){
//                         if(error){
//                             res.send(id)
//                         }else{
//                             res.send(data)
...
const loadProducts = async (req, res) => {
    ...
    //   const totalProductData = await product.find(query)
      const productData = await product.find(query).sort(sortQuery).limit(limit*1).skip((page-1)*limit)
...
const loadDetailsViewProduct = async(req,res)=>{
    if(req.session.user_id){
        ...
        const categoryData = await category.find({is_delete:false})
        const allSizes = await product.schema.path('sizes').enumValues
        const orderData = await order.find({user:req.session.user_id,'product.productId':req.query.id})
        console.log(orderData);
        const orderLength = orderData.length
        console.log(orderLength);
        // console.log(userData);
        res.render('productDetails',{userData,productData,categoryData,allSizes,orderLength})
    }else{
        const productData = await product.findById({_id:req.query.id})
        const categoryData = await category.find({is_delete:false})
        const allSizes = await product.schema.path('sizes').enumValues
...
const loadCart = async(req,res)=>{
    try {
        if(req.session.user_id){
            ...
            const categoryData = await category.find({is_delete:false})
            const userData = await users.find({_id:req.session.user_id}).populate('cart.items.productId')
...
async function updateCartQuantity(userId,proId,count){
    const updatedCart = await users.findOneAndUpdate({
        _id: userId,
...
const updateCart = async(req,res)=>{
    try {
       ...
       if(count == -1){
        ...
        let data = await users.find({_id:req.session.user_id}).populate('cart.items.productId')
           console.log(data[0].cart.items[0].productId.price);
           console.log('helllooo');
            let product = data[0].cart.items.find((product)=>{
                
                return new String(product.productId._id).trim() == new String(proId).trim()
            })
            ...
            if(product.quantity <= 1){
                ...
                res.json({delete:true,total:updatedUser.cart.totalPrice,user:updatedUser})
            }else{
                console.log('decrease the quantity');
                const updatedCart =await updateCartQuantity(req.session.user_id,proId,count)
...
async function deleteItem(userId,proId){
    const userData = await users.findOneAndUpdate({_id:userId},{$pull:{'cart.items':{productId:proId}}},{new:true});
    return userData
}
        ...
        return new Promise((resolve,reject) =>{
            try {
                ...
                if(req.session.user_id){
                    ...
                    Promise.all([userData,categoryData]).then(([userData,categoryData])=>{
...
const updateProfile = async(req,res)=>{
    try {
        ...
        const email = req.body.email
        const userData = await users.findOneAndUpdate({_id:req.session.user_id},{
        firstName:firstName,
        lastName:lastName,
        email:email,
...
const loadWishList = (req,res)=>{
    try {
        return new Promise((resolve,reject)=>{
            if(req.session.user_id){
                const userData = users.findById(req.session.user_id).populate('wishList.productId')
                const categoryData = category.find({is_delete:false})
                Promise.all([userData,categoryData]).then(([userData,categoryData])=>{
...
const wishListToCart =async(req,res)=>{
    try {
        ...
        const productData = await product.findById(pId)
        let push = false;
        const userData = await users.findById({_id:req.session.user_id}).populate('cart.items.productId')
      
        const cartExit = userData.cart.items.findIndex(product =>{
            return new String(product.productId._id).trim() == new String(productData._id).trim()
        })
...
const deleteFromWishList = async(req,res)=>{
    try {
        ...
        users.findByIdAndUpdate(req.session.user_id,{$pull:{wishList:{productId:pid}}},{new:true})
        .exec((error,data)=>{
            if(error){
                console.log(error.message);
            }else{
